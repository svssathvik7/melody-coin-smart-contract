...
contract MelodyCoinTest is Test {
    ...
    function test_FuzzyTokenDetails(
        ...
        vm.startPrank(sathvik);
        MelodyCoin newCoin = new MelodyCoin(name, symbol, decimals, initialSupply, maxCap);
        vm.stopPrank();
        ...
        assertEq(newCoin.totalSupply(), initialSupply, "Initial supply not set correctly");
    }
}
